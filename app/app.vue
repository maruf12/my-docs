...
const { data: navigation } = await useAsyncData(
    ...
    queryCollectionNavigation(collectionName.value as keyof PageCollections),
  {
    transform: (data: ContentNavigationItem[]) => {
      ...
      return (
        rootResult.find(
          (item: ContentNavigationItem) => item.path === `/${locale.value}`,
        )?.children || rootResult
      );
    },
    watch: [locale],
...
// Use custom search that includes all file types (not just .md)
const { data: files } = useLazyAsyncData(
  `search_${collectionName.value}`,
  () => useCustomSearchSections(collectionName.value as keyof PageCollections),
  {
    server: false,
    watch: [locale],
  },
...
<template>
  <UApp :locale="nuxtUiLocale">
    ...
    <div
      ...
      :style="{
        marginRight: shouldPushContent ? `${assistantPanelWidth}px` : '0',
      }"
    >
